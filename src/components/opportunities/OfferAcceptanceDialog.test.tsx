...
describe("offer acceptance dialog", () => {
  it("shows exact terms, message, sender and destination and gates approval on acknowledgement", async () => {
    ...
    render(<OfferAcceptanceDialog descriptor={reviewed} onApprove={onApprove} onOpenChange={vi.fn()} open />, { wrapper: LocaleProvider });
  ...
  it("clears acknowledgement when the reviewed snapshot changes", () => {
    const onApprove = vi.fn().mockResolvedValue(undefined);
    const { rerender } = render(<OfferAcceptanceDialog descriptor={descriptor()} onApprove={onApprove} onOpenChange={vi.fn()} open />, { wrapper: LocaleProvider });
  ...
  it.each([
    [{ current: false }, /no longer current/i],
    [{ expiresAt: 1 }, /approval has expired/i],
    [{ status: "failed" }, /failed and was not confirmed as sent/i],
  ])("blocks approval for stale, expired, or failed data", (overrides, message) => {
    render(<OfferAcceptanceDialog descriptor={descriptor(overrides)} onApprove={vi.fn()} onOpenChange={vi.fn()} open />, { wrapper: LocaleProvider });
  ...
  it("shows loading and preparation errors without presenting approval controls", () => {
    const { rerender } = render(<OfferAcceptanceDialog loading onApprove={vi.fn()} onOpenChange={vi.fn()} open />, { wrapper: LocaleProvider });
  ...
  it("surfaces send errors and requires a fresh acknowledgement", async () => {
    render(<OfferAcceptanceDialog descriptor={descriptor()} onApprove={vi.fn().mockRejectedValue(new Error("ACCEPTANCE_CONTENT_CHANGED"))} onOpenChange={vi.fn()} open />, { wrapper: LocaleProvider });
  ...
  it.each(["approved", "queued", "executing"])("keeps %s delivery non-reviewable and does not claim it was sent", (status) => {
    render(<OfferAcceptanceDialog descriptor={descriptor({ status, current: false })} onApprove={vi.fn()} onOpenChange={vi.fn()} open />, { wrapper: LocaleProvider });
    expect(screen.getByRole("status")).toHaveTextContent("Delivery is still being checked");
    expect(screen.getByRole("checkbox")).toBeDisabled();
    expect(screen.getByRole("button", { name: "Approve and send acceptance" })).toBeDisabled();
