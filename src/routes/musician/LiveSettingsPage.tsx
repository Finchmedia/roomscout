...
export function LiveSettingsPage() {
  ...
  >
    ...
    {!loading && page === "sources" ? <LiveSourcesSection
      ...
      portalActions={(portal) => <>{portal.status === "active" && portal.allowInboxPolling ? <Button size="xs" variant="secondary" disabled={Boolean(working)} onClick={() => void run(`sync:${portal._id}`, () => syncInbox({ connectionId: portal._id }), copy("synced"))}>{copy("syncInbox")}</Button> : null}{portal.status === "active" ? <Button size="xs" variant="secondary" disabled={Boolean(working)} onClick={() => void run(`pause:${portal._id}`, () => pauseConnection({ connectionId: portal._id }), copy("connectionPaused"))}>{copy("pause")}</Button> : null}{["needs_auth", "reauth_required", "paused"].includes(portal.status) ? <Button size="xs" disabled={Boolean(working)} onClick={() => void run(`auth:${portal._id}`, async () => { const result = await startAuthentication({ connectionId: portal._id }); navigate(`/app/runs/${result.runId}`); }, copy("authenticationStarted"))}>{copy("authenticate")}</Button> : null}{portal.status === "needs_auth" && isControlledDemoUrl(portal.baseUrl) ? <Button size="xs" variant="secondary" disabled={Boolean(working)} onClick={() => void run(`register:${portal._id}`, async () => { const result = await startRegistration({ connectionId: portal._id }); navigate(`/app/runs/${result.runId}`); }, copy("registrationStarted"))}>{copy("registerScout")}</Button> : null}<Button size="xs" variant="danger" disabled={Boolean(working)} onClick={() => setDisableId(portal._id)}>{copy("remove")}</Button></>}
