...
function setup() {
  ...
  render(<LiveKnowledgeSection summary="Eine Band auf Raumsuche." facts={facts} events={[]} busy={false} {...actions} />);
  return actions;
}
...
describe("LiveKnowledgeSection", () => {
  it("derives current-search budget, currency, period and concise genres without a memory budget", () => {
    ...
    expect(combined.map(displayText)).toEqual(expect.arrayContaining(["Bis 400 € / Monat", "Rock", "Raumteilung möglich"]));
  ...
  it("keeps all actual location, radius, schedule and requirement values", () => {
    ...
    expect(combined.map(displayText)).toEqual(expect.arrayContaining(["Stuttgart", "20 km Umkreis", "Dienstag flexibel · 1x wöchentlich", "Laute Proben mit PA erforderlich"]));
  });
