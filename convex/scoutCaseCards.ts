...
export function buildScoutCaseCard(input: CaseCardInput): string {
  ...
  if (input.mode === "search_discovery") {
    ...
    const lifecycle = status !== undefined && status !== "draft"
      ...
      : `READY HANDOFF: Once the draft is useful enough to run and material ambiguity is resolved, first apply any final updates, then call markSearchBriefReady and say in ONE sentence that the brief is ready for review and that only the musician can start the search in the app. Describe that action in the musician's current language; do not quote a UI button label. Do not merely say it is complete without the successful tool result, and do not require every optional field.`;
...
QUESTION GATE: Ask only about a material search gap that remains unknown after checking the musician's current words, durable memory and every non-unknown field in Active search. An explicit statement answers that point immediately; a saved value remains answered. Never ask the same point again in broader or rephrased form. In particular, arrangements=shared already answers whether they are open to sharing. Wanting to leave gear is a storage requirement; it does not create a separate security question.
