...
async function fixture() {
  ...
  const ids = await t.run(async (ctx) => {
    ...
    const mandateId = await ctx.db.insert("searchMandates", { ownerId, savedNeedId: needId, version: 1, mode: "negotiation_autopilot", status: "active", platformIds: [platformId], allowedActionTypes: ["send_email"], allowedPersonalData: ["reply_email"], maxContactsPerDay: 5, maxBrowserMinutesPerDay: 30, maxMonthlyPriceEur: 300, expiresAt: now + 86_400_000, stopOnComplaint: true, stopWhenSuitableRoomConfirmed: true, commitmentBoundary: "non_binding_outreach_only", contentHash: "mandate-v1", createdAt: now, updatedAt: now });
    ...
    return { ownerId, otherId, needId, policyId, bindingId, mandateId, mailboxId, threadId, conversationId, offerId };
  });
